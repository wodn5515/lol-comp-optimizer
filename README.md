...
- **단계적 프론트라인 평가**: 조합 유형별 등급제 (필수/불필요/유연), 탱커·브루저 수에 따른 차등 점수
- **적 조합 카운터**: 적 3픽 이상 시 상성 분석 (유리 +8, 불리 -5), 12가지 조합 유형 간 상성표 적용
- **실시간 밴픽**: 챔피언 밴, 적 팀 픽, 아군 고정 픽, **포지션 고정** 반영 → 실시간 추천 업데이트
- **메타 챔피언 자동 보충**: 밴/적픽으로 풀 부족 시 S>A>B 티어 순으로 메타 챔피언 추가
- **추천 다양성**: 조합 간 최소 챔피언 차이 보장 + 2가지 이상 조합 유형 포함
- **점수 분해**: "왜 이 조합인지" 항목별 점수 시각화 (숙련도, 메타, 밸런스, 프론트라인 등)
- **접이식 조합 카드**: 1위 자동 펼침, 나머지 접힘 → 스크롤 절약
- **플렉스 픽 감지**: 2개+ 라인 가능 챔피언 자동 표시 (드래프트 전략 활용)
- **클립보드 복사**: 추천 조합을 로비 채팅 형식으로 원클릭 복사
- **세션 유지**: 새로고침해도 분석 결과·밴픽 상태 유지 (30분 자동 만료)
- **멀티서치**: 롤 로비 채팅 복붙으로 소환사 자동 입력
- **챔피언 한글 표시**: 171개 챔피언 한글 이름, 가나다순 정렬, 한글+영문 검색
- **챔피언 운영 팁**: 171개 챔피언별 콤보/파워 스파이크/핵심 플레이 한글 가이드
...
**프론트라인 등급제:** 조합 유형별 차등 (필수 조합: 0명=0점~2명=100점, 불필요 조합: 0명=80점)

**추가 감점:**
- 풀 AD/AP: -30점 / 웨이브클리어 부족: -10점
...
│   ├── champion_attributes.json # 171개 챔피언 데이터 (속성, 한글 이름, 메타 티어, 운영 팁)
│   └── tests/                   # 119개 테스트 (unit + integration + e2e)
